...
public class SpaceExplorer extends Thread {
	...
	public void run() {
		for( ; ; ){
			Message messageFromHQs = channel.getMessageHeadQuarterChannel();		//iau mesaj de la HQs

			if(messageFromHQs.getData().compareTo(HeadQuarter.EXIT) == 0)
				return;

			if(messageFromHQs.getData().compareTo(HeadQuarter.END) == 0)
				continue;	

			//Message messageFromHQs1 = channel.getMessageHeadQuarterChannel();		//iau mesaj de la HQs
	
			Integer parrentSolarSystem = messageFromHQs.getParentSolarSystem();
			Integer currentSolarSystem = messageFromHQs.getCurrentSolarSystem();

			if(!discovered.contains(currentSolarSystem)){
				discovered.add(currentSolarSystem);
			
				String data = messageFromHQs.getData();
				String encodedData = encryptMultipleTimes(data, hashCount);
		
				channel.putMessageSpaceExplorerChannel(new Message(parrentSolarSystem, currentSolarSystem, encodedData));
			}
